...
void MainWindow::save(){
    //Only save to file if their is student(s) in object.
    if(studentList->size() > 0){
        QFile file("studentList.xml");
        if (!file.open(QIODevice::WriteOnly)){
            QMessageBox::warning(this, "Student List", "Couldnt save to file.");
            return;
        }

        QTextStream fileStream(&file);
        XMLWriter xmlw;
        foreach(Student * student , studentList->returnList()){
              xmlw.addStudent(student);
        }

        QDomDocument doc = xmlw.getDoc();
        fileStream << doc.toString();
        file.close();
        QMessageBox::information(this, "Student List", "File saved with new students in List.");
    }
}

//Close Event
void MainWindow::closeEvent(QCloseEvent *event){
QMessageBox::StandardButton close = QMessageBox::question( this, "Process Student", tr("Are you sure?\n"),
